...
public abstract class BasicClient {
    ...
    protected <T, U extends service.retrofit.Response> T getResponse(Call<T> call, Class<U> errorModel) {
        return getOrThrowException(call, errorBody -> getApiErrorText(errorBody, errorModel));
    }

    private <T extends service.retrofit.Response> String getApiErrorText(ResponseBody body, Class<T> model) throws IOException {
        String payload = body.string();
        if (!Utils.isBlankString(payload)) {
            try {
                service.retrofit.Response response = gson.fromJson(payload, model);
                //service.retrofit.Response response = objectMapper.readValue(payload, model);
